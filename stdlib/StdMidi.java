 ...
 *  The {@code filename} must have the extension {@code .mid} or {@code .midi}.
 *  <p>
 *  Bug 1: the current version of {@code save()} does not currently
 *  save the percussion track.
 *  <p>
 *  Bug 2: too many instrument changes breaks MIDI recording because MIDI supports
 *  only 16 tracks.
 *
 *
 *  <p>
...
public class StdMidi {
    ...
    private static final int PERCUSSION_CHANNEL_NUMBER = 9;  // MIDI channel 10 (1-based indexing)
    ...
    static {
        ...
        Synthesizer openedSynthesizer = null;

        try {
            openedSynthesizer = MidiSystem.getSynthesizer();
            openedSynthesizer.open();
            MidiChannel[] channels = openedSynthesizer.getChannels();

            // Publish fields only after all validation succeeds.
            synthesizer = openedSynthesizer;
            channel = channels[0];
            percussionChannel = channels[PERCUSSION_CHANNEL_NUMBER];
            ...
            if (openedSynthesizer != null && openedSynthesizer.isOpen()) {
                openedSynthesizer.close();
            }
            System.err.println("Couldn't open a MIDI device");

            throw new ExceptionInInitializerError("Unable to initialize MIDI synthesizer: " + e.getMessage());
    ...
    private static void startRecording() {
        try {
            ...
            throw new IllegalStateException("Unable to initialize MIDI recording", e);
        }
        catch(Exception e) {
            throw new IllegalStateException("Unable to initialize MIDI recording", e);
        }
    ...
    public static void setInstrument(int instrument) {
        ...
        channel.programChange(instrument - 1);  // MIDI uses 1=based indexing; Java uses 0-based indexing
        ...
        try {
            ...
            track = sequence.createTrack();
            channelNumber++;
            if (channelNumber == PERCUSSION_CHANNEL_NUMBER) {
                channelNumber++;
            }
            if (channelNumber > 15) {
                throw new IllegalStateException( "No MIDI channels remain for another instrument");
            }

            ShortMessage instrumentChange = new ShortMessage(ShortMessage.PROGRAM_CHANGE, channelNumber, instrument - 1, MIDI_MESSAGE_IGNORE_BYTE);
    ...
    public static void pause(double beats) {
        ...
        tick += Math.round(MIDI_TICKS_PER_QUARTER_NOTE * beats);
    }
    ...
    /**
     * Turns all notes and percussion off, but allows decaying notes to complete.
     */
    public static void allNotesOff() {
        channel.allNotesOff();
        percussionChannel.allNotesOff();
    }

    /**
     * Turns all notes and percussion off immediately.
     */
    public static void allSoundOff() {
        channel.allSoundOff();
        percussionChannel.allSoundOff();
    }
